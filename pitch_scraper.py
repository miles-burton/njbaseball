#!/usr/bin/env python3
"""Pitch Index soccer data scraper for NJ.com boys and girls soccer."""
...
GENDER = "girls" if SPORT == "girlssoccer" else "boys"
GENDER_LABEL = "Girls" if GENDER == "girls" else "Boys"
BASE = "https://highschoolsports.nj.com"
OUTPUT_PATH = os.environ.get(
    "PITCH_OUTPUT_PATH",
    "js/girls-pitch-data.js" if GENDER == "girls" else "js/pitch-data.js",
)
HEADERS = {
...
def team_key(value):
    ...
    value = re.sub(r"\b(high|school|boys|girls|soccer|prep|preparatory|academy|regional|the)\b", " ", value)
    value = re.sub(r"[^a-z0-9]+", " ", value)
    return re.sub(r"\s+", " ", value).strip()


def parse_conference(conf):
    conference_slug = "Big-North" if SPORT == "girlssoccer" and conf == "Big North" else conf
    url = f"{BASE}/{SPORT}/standings/season/{SEASON}?conference={urllib.parse.quote(conference_slug)}"
    html = fetch(url)
    ...
    for chunk in re.split(r"(<thead.*?</thead>)", table, flags=re.S | re.I):
        ...
        for row in re.findall(r"<tr.*?</tr>", chunk, flags=re.S | re.I):
            href_match = re.search(rf'href="(/school/([^/]+)/{re.escape(SPORT)}/season/[^"]+)"', row)
            if not href_match:
                continue
...
def parse_schedule(html, team_slug):
    ...
    for row in re.findall(r"<tr.*?</tr>", table_match.group(0), flags=re.S | re.I)[1:]:
        ...
        href = re.search(rf'href="(/school/([^/]+)/{re.escape(SPORT)}/season/[^"]+)"', opponent_cell)
...
def main():
    ...
    print(f"Fetching {SEASON} {GENDER} soccer conference standings...")
    all_teams = []
    seen = set()
    conference_counts = {}
    for conf in CONFERENCES:
        try:
            ...
            conference_counts[conf] = len(teams)
            ...
            conference_counts[conf] = 0
            print(f"  {conf}: ERROR {exc}")
        time.sleep(0.08)

    missing_conferences = [conf for conf, count in conference_counts.items() if count == 0]
    if missing_conferences:
        raise RuntimeError(f"Refusing to publish incomplete NJ.com data: no teams found for {', '.join(missing_conferences)}")
    ...
    with open(temp_path, "w", encoding="utf-8") as f:
        f.write(f"// Auto-generated by pitch_scraper.py from highschoolsports.nj.com {GENDER} soccer data.\n")
        f.write("const PITCH_DATA = ")
        json.dump(payload, f, ensure_ascii=False, separators=(",", ":"))
        f.write(";\n")
    os.replace(temp_path, OUTPUT_PATH)
    print(f"Wrote {OUTPUT_PATH} with {len(payloads)} teams, {len(scorers)} scorers, {len(keepers)} keepers, {len(player_logs)} player logs.")
